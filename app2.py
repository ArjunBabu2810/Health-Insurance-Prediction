...
def explain_prediction(data):
    """
    Uses SHAP to explain the prediction for the given input data.
    Returns SHAP values and base value.
    """
    ...
    # Use SHAP TreeExplainer for GradientBoostingRegressor
    explainer = shap.Explainer(MODEL, final_features)
...
def explain():
    """
    Returns SHAP values for the input data to explain the prediction.
    """
    if not request.json:
        return jsonify({'error': 'No JSON payload provided'}), 400
    ...
    try:
        ...
        return jsonify({'error': 'Explanation failed due to internal server error.'}), 500
